...
class MovieLensDC(DecisionTree):
    ...
    def display_scores_dc(self, scores):
        self.validation_acc_dc =  scores['test_accuracy']
        if not self.multiclass:
            self.validation_recall_dc =  scores['test_recall']
            self.validation_precision_dc =  scores['test_precision']
        self.train_acc_dc =  scores['train_accuracy']
        if not self.multiclass:
            self.train_recall_dc =  scores['test_recall']
            self.train_precision_dc =  scores['train_precision']
        self.estimators_dc = scores['estimator']
        best_acc = 0.0
        best_acc_index = 0
        for acc in self.validation_acc_dc:
            if acc > best_acc:
                best_acc = acc
                best_acc_index = np.where(self.validation_acc_dc == best_acc)  
        self.best_model_index_dc = best_acc_index[0][0] 
        # self.feature_importance = self.estimators[self.best_model_index].feature_importances_.tolist()
        # self.feature_importance_info()
        # print(self.features)
        # print(self.feature_importance)        
        ...
        if not self.multiclass:
            print('==== Recal(DC) #%d ==== \n' % self.round)
            print(scores['test_recall'])
            print('==== Precision(DC) #%d ==== \n' % self.round)
            print(scores['test_precision'])
            # print('==== NMSE #%d ==== \n' % self.round)
            # print(scores['test_neg_mean_squared_error'])        
        print('==== Train Accuracy(DC) #%d ==== \n' % self.round)
        print(scores['train_accuracy'])   
        if not self.multiclass:
            print('==== Train Recall(DC) #%d ==== \n' % self.round)
            print(scores['train_recall'])
            print('==== Train Precision(DC) #%d ==== \n' % self.round)
            print(scores['train_precision']) 
            # print('==== Train NMSE #%d ==== \n' % self.round)
            # print(scores['train_neg_mean_squared_error'])                
        print("\nBest model index(DC): %d \n" % self.best_model_index_dc)

    def train(self):
        X, Y = self.XY(self.training_set)
        # print(X.head(n=20))
        # sys.exit(0)
        dc = KNeighborsClassifier(p=1, n_neighbors=1)
        dc1 = DecisionTreeClassifier(criterion='gini', max_depth=self.depth, min_samples_leaf=self.min_leaf_samples)
        if self.multiclass:
        ...
        self.display_scores_dc(scores1)
        # if self.dctree:
    ...
    def test(self):
        ...
        y_predict_dc = self.estimators_dc[self.best_model_index_dc].predict(X)
        ...
        self.test_precision_dc = precision_score(Y, y_predict_dc, average='macro')
        self.test_recall_dc = recall_score(Y, y_predict_dc, average='macro')
        self.test_acc_dc = accuracy_score(Y, y_predict_dc)
        print(' Best Model Accuracy(DC): %f' % self.test_acc_dc)        
    ...
    def round_summary(self):
        if not self.multiclass:
            ...
            round_summary = {
                ...
                "test_recall": self.test_recall,
                "test_precision": self.test_precision,
                "best_model_index_dc": int(self.best_model_index_dc),
                "validation_acc_dc": self.validation_acc_dc.tolist(),
                "train_acc_dc": self.train_acc_dc.tolist(),
                "test_acc_dc": self.test_acc_dc,
                "test_recall_dc": self.test_recall_dc,
                "test_precision_dc": self.test_precision_dc,                
            }                
        print(round_summary)
...
def report(args):
    ...
    test_acc_list_dc = []
    # test_recall_list_dc = []
    # test_precision_list_dc = []
    validation_acc_list_dc = []
    train_acc_list_dc = []    
    for rs in data_list:
        ...
        test_acc_list_dc.append(rs['test_acc_dc']) 
        # test_recall_list_dc.append(rs['test_recall_dc'])
        # test_precision_list_dc.append(rs['test_precision_dc'])
        validation_acc_list_dc.append(rs['validation_acc_dc'][rs['best_model_index_dc']])
        train_acc_list_dc.append(rs['train_acc_dc'][rs['best_model_index_dc']])
    accuracies = pd.DataFrame({
        "test accuracy knn": test_acc_list,
        "test accuracy dc": test_acc_list_dc,
    }, index = [
        'R1', 'R2',
        'R3', 'R4',
        'R5', 'R6',
        'R7', 'R8',
        'R9', 'R10'])
    pearson_coef = accuracies.corr()
    print(pearson_coef)
    
    # print(report_data.head(n=10))
    # print("test mean accuracy: %f" % report_data['test accuracy'].mean())
    acc_lines = accuracies.plot.line(rot=0)
    # barchart.set_ylim(0.2, 1.0)
    plt.savefig(os.path.join(output_folder, "acc_lines.png"))
